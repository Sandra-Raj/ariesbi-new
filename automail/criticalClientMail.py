...
from config import EMAIL_CONFIG, DATA_PATH
...
def fetch_outstanding_debtors(file_path):
    ...
    query = f"""
        SELECT 
            ...
            END AS bal_6m,
            CASE 
                WHEN t.tran_date >= (CURRENT_DATE - INTERVAL '3 months') 
                THEN (t.ov_amount - t.alloc) * t.rate 
                ELSE 0 
            END AS bal_3m,
...
def process_debtor_stats(raw_data):
    ...
    for row in raw_data:
        ...
        clients[name]['bal_3m'] += row['bal_3m']
...
def generate_html_table(processed_data):
    ...
    for i, row in enumerate(processed_data, start=1):
        ...
        <tr>
            ...
            <td style="border:1px solid #ddd; padding:8px; text-align:right; color: {'#d9534f' if row['bal_3m'] == row['bal_6m'] else '#5cb85c'};">{row['bal_3m']:,.2f}</td>
    ...
    <tr style="background-color: #eee; font-weight: bold;">
        ...
        <td style="border:1px solid #ddd; padding:8px; text-align:right;">{grand_total_3m:,.2f}</td>
        ...
        <table style="border-collapse: collapse; width: 100%; font-family: sans-serif;">
            <thead style="background-color: #f2f2f2;">
                <tr>
                    ...
                    <th>Outstanding (3 months, AED)</th>
...
# --- Execution ---
raw_data = fetch_outstanding_debtors(DATA_PATH)
debtor_data = process_debtor_stats(raw_data)

if debtor_data:
    send_notification(debtor_data)
else:
    print("No outstanding transactions found in the last 6 months.")